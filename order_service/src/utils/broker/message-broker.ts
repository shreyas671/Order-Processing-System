...
const createTopic = async (topic: string[]) => {
    const topics = topic.map((t) => ({
        topic: t,
        numPartitions: 2,
        replicationFactor: 1 //based on the available brokers
...
const connectProducer = async <T>(): Promise <T> => {
    ...
    if(producer) {
        console.log("Producer is already connected with an existing connction");
...
const publish = async (data: PublishType) : Promise<boolean>  => {
    ...
    console.log("Publishing Resut", result);
